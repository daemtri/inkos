...
InkOS 是一个面向故事创作与多语言翻译的 AI Agent 系统：长篇连载、独立短篇、剧本剧作、同人番外、仿写续写、互动影游、开放世界和长文翻译，都可以从同一个工作台开始。支持 Studio、TUI、CLI 交互形式，把创意、设定、角色、记忆、审稿、修订、互动状态和跨语言交付交给智能体统一管理。
...
- **模型配置** — Studio 内置多服务配置、模型路由和配图服务配置；也支持 [kkaiapi](https://kkaiapi.com/) / OpenRouter 等全球主流模型聚合入口，以及自定义 OpenAI-compatible 服务。
...
InkOS Play 发布和 Studio 体验升级：你可以用一句自然语言创建开放世界，让角色、物品、证据、关系和时间一起推进；也可以继续写长篇、做短篇、改设定和查状态。系统会记住世界发生了什么，并在需要时把该看的上下文带给模型。
...
- **指令遵循**：Studio Chat、TUI 和 CLI 的自然语言入口统一到 action surface。普通讨论、确认建书、短篇、Play、长篇写章和重写续写不再靠散落关键词抢跑，重动作先确认，完成态来自真实工具结果。
- **创作入口**：长篇、短篇、同人、番外、仿写、续写和开放世界都成为 Studio 的一等入口。
...
**InkOS Short** — Studio Chat 和 CLI 可以直接产出独立短篇：完整正文、大纲记录、审稿记录、简介卖点。

**InkOS Play** — 新增开放世界与分支互动。你可以用自然语言指定世界契约、时间推进方式、角色 agent、物品 / 证据 / 关系规则和视觉风格；系统维护世界状态、可点击选择、自由动作、HUD 和自动配图。

**Studio Chat** — 普通聊天、建书、短篇、互动世界都走同一套 action surface。重动作先确认，生成物可预览，可通过聊天修改章节、世界状态和持久化文本产物。
...
- **Studio Chat**：讨论、建书、短篇、Play、编辑持久化文件都从同一个对话入口发起；重动作会先展示确认卡。
...
生成物会落在 `shorts/<故事名>/final/`，包含 `full.md`、`sales-package.md`。
...
系统会生成世界、角色、物品、证据、关系、当前场景和可选动作。开放世界支持自由输入动作；分支互动会给出可点击选项。配置图片服务后，角色、物品、证据、场景都可以生成图，并在对话流里滚动显示。
...
Studio Chat 不再只是问答框。它可以创建长篇、跑短篇、启动 Play、编辑持久化文本文件，并在需要执行重动作前给出确认。普通讨论会直接回答；明确创作动作才进入工具执行。
...
Agent 模式暴露的是按场景收窄后的工具集：建书、读写控制面、规划、编排、写作、审稿、修订、短篇、Play 等能力会按当前 session 类型开放。推荐的 Agent 工作流是：先调整控制面，再 `plan` / `compose`，最后决定写草稿还是跑完整管线。
...
第一张是当前 Studio 的本地实测截图。后面三张来自 InkOS Play 的真实本地生成结果，用于展示开放世界、侦探证据、互动场景和物品视觉能力。
...
| `inkos short run`                           | 生成独立短篇包（正文、简介卖点）                                                               |
